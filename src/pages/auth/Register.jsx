...
function Register() {
  ...
  useEffect(() => {
    if (message === "Account created sucessfully, you can login now") {
      setTimeout(() => {
        history.push("/login");
      }, 1500);
    }
  ...
  return (
    <HomeLayout>
      <div className="bg-gray-50 dark:bg-gray-900 min-h-screen flex flex-row items-center w-full">
        <div className="login flex flex-col items-center w-full">
          <Text className="text-2xl mb-4 font-semibold">Sign Up</Text>
          <button
            className="bg-blue-900 hover:bg-blue-800 self-center md:w-2/5 w-4/5 justify-center rounded p-2 flex flex-row items-center "
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm my-2">
            Or register using credentials
          </p>

          {message ? (
            <p className="bg-blue-200 border-l-4 border-blue-600 md:w-2/5 w-4/5 text-gray-700 capitalize font-semibold text-center p-2 rounded-sm">
              {message}
            </p>
          ) : null}
          {err ? (
            <p className="bg-red-200 border-l-4 border-red-600 md:w-2/5 w-4/5 text-gray-700 capitalize font-semibold text-center p-2 rounded-sm">
              {err}
            </p>
          ) : null}
          {error ? (
          ...
          {!loading ? (
            <button
              onClick={registerWithCreds}
              className={`bg-yellow-400 hover:bg-yellow-500 self-center font-semibold w-2/5 mt-8 justify-center rounded text-gray-700 p-2 flex flex-row items-center`}
            >
              Sign Up
            </button>
          ) : (
            <button
              disabled
              className={`bg-yellow-400 opacity-50 self-center font-semibold w-2/5 mt-8 justify-center rounded text-gray-700 p-2 flex flex-row items-center`}
            >
              Sign Up
            </button>
          )}
